...
describe("API server", () => {
  it("accepts whale signals and exposes them", async () => {
    ...
    const createResp = await request(app).post("/api/v1/signals/whale").send({
      wallet: "0xabc",
      token: "ETH",
      side: "buy",
      sizeUsd: 100000,
    ...
    const listResp = await request(app).get("/api/v1/signals/whale?status=pending");
    expect(listResp.status).toBe(200);
    expect(listResp.body.items.length).toBe(1);

    const shareResp = await request(app).get("/api/v1/growth/share/latest");
  ...
  it("updates strategy profiles and exports backtest snapshot", async () => {
    ...
    const profileUpdate = await request(app)
      .post("/api/v1/strategies/profile")
      .send({ strategyId: "dex-arbitrage", variant: "B", params: { notionalMultiplier: 1.4 } });
    expect(profileUpdate.status).toBe(200);

    const profileList = await request(app).get("/api/v1/strategies/profiles");
    expect(profileList.status).toBe(200);
    expect(profileList.body.items.length).toBeGreaterThan(0);

    const snapshotJson = await request(app).get("/api/v1/backtest/snapshot?hours=24");
    expect(snapshotJson.status).toBe(200);
    expect(snapshotJson.body.rows.length).toBeGreaterThan(0);

    const snapshotCsv = await request(app).get("/api/v1/backtest/snapshot?hours=24&format=csv");
    ...
    const replay = await request(app)
      ...
      .send({ seed: "demo-seed", hours: 24, mode: "paper" });
    expect(replay.status).toBe(200);
    expect(replay.body.seed).toBe("demo-seed");
    expect(replay.body.total).toBeGreaterThan(0);

    const integrationStatus = await request(app).get("/api/v1/integration/onchainos/status");
    expect(integrationStatus.status).toBe(200);
    expect(integrationStatus.body.authMode).toBe("bearer");

    const probe = await request(app)
      .post("/api/v1/integration/onchainos/probe")
      .send({ pair: "ETH/USDC", chainIndex: "196", notionalUsd: 25 });
    expect(probe.status).toBe(503);
    expect(probe.body.ok).toBe(false);
    expect(probe.body.configured).toBe(false);

    const tokenCache = await request(app).get("/api/v1/integration/onchainos/token-cache?symbol=ETH&chainIndex=196");
    expect(tokenCache.status).toBe(200);
    expect(tokenCache.body.items.length).toBe(1);
